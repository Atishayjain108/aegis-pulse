...
- **Market Intelligence currency = USD base (2026-06-23)**: `ProductIntelligenceEngine.analyze()` calls `_normalize_currencies(products)` (live ECB/Frankfurter via `aegis.geo.fx.FXRateFetcher`) BEFORE `_synthesize`. Every priced `ProductRecord` gets `price_usd`; ALL comparison/arbitrage/band/pick/value math uses `price_usd`, never the raw `price` (which is per-marketplace currency). This killed the false cross-currency arbitrage bug (₹ vs $ compared raw). Display keeps original `price`+`currency` plus `price_usd`. When FX is unreachable AND all listings share one currency, `price_usd` falls back to raw `price` (a monotonic transform — rankings/gap% identical).
- **Market Intelligence data-status contract**: `MarketReport.status` ∈ {`OK`, `INSUFFICIENT_EVIDENCE`, `NO_DATA`} + `status_reason`, stamped by `_apply_status()`. `NO_DATA` (0 listings AND 0 demand mentions) → no synthesized executive_summary/picks; the summary IS the reason. Below the floor (`_MIN_PRODUCTS_FLOOR=3` / `_MIN_PRICED_FLOOR=2`) → `INSUFFICIENT_EVIDENCE`. The LLM verdict is ONLY requested when `status=="OK"`; otherwise a deterministic `RECOMMENDATION: INSUFFICIENT_EVIDENCE` is emitted — the model is never asked to commit to a call without an evidence floor.
- **Market Intelligence provenance banding**: every `evidence` entry carries `band` ∈ {`VERIFIED` (direct from scraped listings), `ESTIMATED` (model/proxy from verified inputs — FX-converted prices, momentum heuristics), `UNVERIFIED`}. `_ground_narrative()` post-checks the LLM output: any figure ≥10 not within 1% of a known report number is appended as an explicit `⚠ GROUNDING WARNING` (flagged, not silently trusted).
- **Brand-doubling fix**: `aegis.scrape.ecommerce_utils.join_brand_title(brand, name)` skips the brand prefix when `name` already starts with it ("ETUDE" + "ETUDE Lip Gloss" → "ETUDE Lip Gloss", not doubled). Used by myntra/nykaa/ajio adapters. Always use it instead of `f"{brand} {name}"`.
- **Adapter-name resolution tolerant of -/_  (2026-06-23)**: `topic._resolve_registry_entry(name, registry)` matches both `reddit_finance` (file style) and `hacker-news` (registry style) by normalizing `-`↔`_`. The swarm `_REGISTRY` mixes both conventions; an exact `.get()` silently rejected valid adapter overrides. `_build_registry_adapter` goes through this resolver.
- **Reddit adapters trimmed to 5 subreddits (2026-06-23)**: `reddit_finance` / `reddit_ecommerce` had 11 subreddits serialized behind a ~1 req/3s anonymous rate limit → >25s, blowing the 20s per-adapter harvest budget (timed out, returned nothing). Trimmed to 5 + `per_source_rps=0.5`; they now finish under budget. NOTE: from a datacenter IP Reddit still returns 429 on every request (EMPTY) — that is an IP-reputation ceiling, not a speed bug; needs auth or residential IP.
