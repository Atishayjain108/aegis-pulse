...
|---|---|---|---|
| DEBT-1 | `autonomous-image-imports` CI job has never executed on a GitHub runner. The gate is proven at the enforcement layer only (local `docker build`: green run, and deliberately-broken run failing exit 1 with the module named). Until closed, state "a local build gate enforces the import walk; CI is unproven" — never "CI enforces it." | operator (credentials: authorize SSH key, `gh auth login`, or `GH_TOKEN`) | commit `004045f`+ is pushed and BOTH runs — green, then deliberately-red — exist as GitHub run URLs |
