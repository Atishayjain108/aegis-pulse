# AEGIS-EXEC-0011 — Loss probability above ceiling; alert blocked.

**Symbolic name:** `EXEC_RISK_LOSS_PROB_TOO_HIGH`
...
- A risk gate downgraded the verdict to BLOCK.
- The deterministic feature values cross a hard floor or ceiling.
- Check `margin_floor_gate` / `loss_probability_gate` /
  `confidence_gate` thresholds in `constants.py`.

## Diagnosis

1. Search the structlog output for `error_code=AEGIS-EXEC-0011`.
